...
int main()
{
    ...
    for (int l = 1; l <= 4; l++)
    {
        ...
        win1 = newwin(22, 75, 3, 3);
        keypad(win1, true);

        mvwprintw(win1, 0, 7, "Snake Game");
        map.draw(win1, snake, 0);
        ...
        for (int i = 0; i < 3; i++)
        {
            snake_pair[i][0] = snake.getTaily();
            snake_pair[i][1] = snake.getTailx() - i;
        }
        ...
        while ((ch = getch()) != KEY_F(1))
        {
            ...
            if ((float)(end2 - start2) / CLOCKS_PER_SEC >= 5)
            { // feed
                start2 = end2;
            ...
            if ((float)(end3 - start3) / CLOCKS_PER_SEC >= 10)
            { // posion
                start3 = end3;
            ...
            if ((float)(end4 - start4) / CLOCKS_PER_SEC >= 15)
            { // gate
                start4 = end4;
            ...
            if ((float)(end - start) / CLOCKS_PER_SEC >= 0.5)
            {
                ...
                gameend = clock();
                map.draw(win1, snake, (gameend - gamestart) / CLOCKS_PER_SEC);

                if (map.getSuccess() == snake.getSize())
                    break;
